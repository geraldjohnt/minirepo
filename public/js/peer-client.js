/*! peerjs-client build:0.3.15, development. Copyright(c) 2013 Michelle Bu <michelle@michellebu.com> */(function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '"+o+"'");throw f.code="MODULE_NOT_FOUND",f}var l=n[o]={exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e)},l,l.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(require,module,exports){
    ...
    var Reliable = window.Reliable = require('reliable').Reliable;
    ...
    DataConnection.prototype._configureDataChannel = function () {
        ...
        setTimeout(function() {
            switch(self._dc.readyState) {
                case 'open':
                    if(self.trackOnReadyState == 'open') return;

                    self.trackOnReadyState = 'open'
                    util.log('Data channel connection success');
                    self.open = true;
                    self.emit('open');
                break;
                case 'close':
                    if(this.trackOnReadyState == 'open') return;

                    self.trackOnReadyState = 'close'
    ...
    DataConnection.prototype._handleDataMessage = function (e) {
        ...
        if(!this.emit('data', data)) {
            this.emit('fallbackData', data)
        }
    }
    ...
    DataConnection.prototype.send = function (data, chunked) {
        ...
        /**
         * Commented for now, still looking for ways on how to use reliable in sending data
         */
        // if (this._reliable) {
    ...
    DataConnection.prototype.handleMessage = function (message) {
        ...
        switch (message.type) {
            case 'ANSWER':
                ...
                Negotiator.handleSDP(message.type, this, payload.answer);
                break;
            case 'CANDIDATE':
    ...
    function MediaConnection(peer, provider, options) {
        ...
        this.provider = provider;
        this.metadata = this.options.metadata;
        this.localStream = this.options._payload._stream;
    ...
    Negotiator.startConnection = function (connection, options) {
        ...
        if (options.originator) {
            ...
            // Negotiator.handleSDP('OFFER', connection, options.sdp); //Original
            Negotiator.handleSDP('OFFER', connection, options.offer);
        }
    }
    ...
    Negotiator._startPeerConnection = function (connection) {
        ...
        var pc = new window.RTCPeerConnection(connection.provider.options.config, optional);
    ...
    Negotiator._setupListeners = function (connection, pc, pc_id) {
        ...
        pc.oniceconnectionstatechange = function () {
            switch (pc.iceConnectionState) {
                ...
                break;
                case 'disconnected':
                    util.log('iceConnectionState is disconnected, trying reconnect by browser');
                break;
                case 'completed':
        ...
        pc.ondatachannel = function (evt) {
            util.log('Received data channel', evt);
    ...
    Negotiator._makeOffer = function (connection) {
        ...
        pc.createOffer(function (offer) {
            ...
            if (!util.supports.sctp && connection.type === 'data' && connection.reliable) {
                offer.sdp = window.Reliable.higherBandwidthSDP(offer.sdp);
            }
    ...
    Negotiator._makeAnswer = function (connection) {
        ...
        pc.createAnswer(function (answer) {
            ...
            if (!util.supports.sctp && connection.type === 'data' && connection.reliable) {
                answer.sdp = window.Reliable.higherBandwidthSDP(answer.sdp);
            }
    ...
    Negotiator.handleCandidate = function (connection, ice) {
        ...
        connection.pc.addIceCandidate(new window.RTCIceCandidate({
    ...
    function Peer(id, options) {
        ...
        options = util.extend({
            ...
            host: "sign-toky-b-1v6t.webrtc.ecl.ntt.com",
            port: 443,
            key: "{{ env('PEER_API_KEY') }}",
            path: '/',
        ...
        // Start Comment by Terence
        // if (!util.supports.audioVideo && !util.supports.data ) {
        //   this._delayedAbort('browser-incompatible', 'The current browser does not support WebRTC');
        //   return;
        // }
        // End Comment by Terence
    ...
    Peer.prototype._handleMessage = function(message) {
        ...
        var payload = message.payload;
        var peer = payload.src;
        var connection;

        // To keep the connection alive
        this._stopPings();
        clearTimeout(this._pingRequest)
        this._pingRequest = setTimeout(function() {
            this._startPings();
        }.bind(this), 5000);
    
        switch (type) {
            case 'OPEN': // The connection to the server is open.
                this.setupIceServers(payload)
            ...
            case 'OFFER': // we should consider switching this to CALL/CONNECT, but this is the least breaking option.
                ...
                if (connection) {
                    ...
                    if (payload.type === 'media' || payload.connectionType === 'media') {
                    ...
                    } else if (payload.type === 'data' || payload.connectionType === 'data') {
                        connection = new DataConnection(peer, this, {
                            connectionId: connectionId,
                            _payload: payload,
                            metadata: payload.metadata,
                            label: payload.label,
                            serialization: 'json', //payload.serialization,
                            reliable: false //payload.reliable
                        });
                        this._addConnection(peer, connection);
            ...
            break;
            case 'FRESH':
                this.socketConfig = payload;
                if(payload.sid) {
                    this.socket._wsUrl += '&sid=' + payload.sid
                    this.socket._httpUrl += '&sid=' + payload.sid
                    this.socket._socket.url += '&sid=' + payload.sid
                }
            break;
            default:
                if (!payload) {
    ...
    Peer.prototype.setupIceServers = function(payload) {
        const turnCredential = payload.turnCredential
        let turnUserName;
        let turnPassword;;
        ...
        if (this.options.turn === true && turnUserName && turnPassword) {
            ...
            const browser = AdapterJS.webrtcDetectedBrowser;
            if (browser.name !== 'edge') {
                turnCombinations.push({ protocol: 'turns', transport: 'tcp' });
            }

            for (var i=0; i < turnCombinations.length; i++) {
                var turnType = turnCombinations[i];
                ...
                var iceServer = {
                    urls: protocol + ':' + TURN_HOST + ':' + TURN_PORT + '?transport=' + transport,
                    url: protocol + ':' + TURN_HOST + ':' + TURN_PORT + '?transport=' + transport,
                    username: turnUserName,
                    credential: turnPassword,
                };

                this.options.config.iceServers.push(iceServer);
            }
            console.log('SkyWay TURN Server is available');
        } else {
            console.log('SkyWay TURN Server is unavailable');
        }
    }
    
    Peer.prototype._startPings = function() {
        if (!this._pingIntervalId) {
            var interval = 5000;
    
            if(this.socketConfig && this.socketConfig.pingInterval) {
                interval = this.socketConfig.pingInterval
            }
    
            var vm = this;
            this._pingIntervalId = setInterval(function() {
                vm.socket._socket.send('42["PING", null]');
            }, interval);
        }
    };
    
    Peer.prototype._stopPings = function() {
    ...
    Peer.prototype._getSignalingServer = function() {
        var vm = this;
        const http = new XMLHttpRequest();
    
        http.open('GET', 'https://dispatcher.webrtc.ecl.ntt.com:443/signaling', false);
        ...
        http.onreadystatechange = function() {
            ...
            try {
                ...
                console.log(new Error('The dispatcher server returned an invalid JSON response.'));
                return;
            }
    
            if (http.status === 200) {
                if (res && res.domain) {
                    vm.options.host = res.domain;
                }
            }
        };
    ...
    function Socket(secure, host, port, path, key) {
        ...
        this._httpUrl = httpProtocol + host + '/socket.io/?apiKey=' + key;
        this._wsUrl = wsProtocol + host + ':' + port + path + 'socket.io/?apiKey=' + key;
        this.signalingServer = httpProtocol + host + ':' + port;
        this.query = 'apiKey=' + key;
    }
    
    util.inherits(Socket, EventEmitter);
    
    /** Check in with ID or get one from server. */
    Socket.prototype.start = function (id, token, sid) {
        this.id = id;
        this._httpUrl += '&peerId=' + id + '&token=' + token + '&platform=javascript&transport=polling&EIO=3&sdk_version=1.4.0&t=' + yeast()// + '&sid=' + sid;
        this._wsUrl += '&peerId=' + id + '&token=' + token + '&platform=javascript&transport=websocket&EIO=3&sdk_version=1.4.0&t=' + yeast()// + '&sid=' + sid;
        this.query += '&peerId=' + id + '&token=' + token + '&platform=javascript&sdk_version=1.4.0&EIO=3';
    ...
    Socket.prototype._startWebSocket = function (id) {
        ...
        this._socket.extensions = "permessage-deflate; client_no_context_takeover"
        ...
        this._socket.onmessage = function (event) {
            ...
            try {
                if(event.data.indexOf('0{') >= 0) {
                    data = {
                        type: 'FRESH',
                        payload: JSON.parse(event.data.replace('0{', '{'))
                    }
                } else {
                    var packet = socketDataDecoder(event.data);
                    data = {
                        type: packet.data[0],
    ...
    Socket.prototype.ondisconnect = function () {
        console.log('server disconnect (%s)', this.nsp);
        this.destroy();
    ...
    Socket.prototype.onevent = function (packet) {
        ...
        if (null != packet.id) {
            console.log('attaching ack callback to event');
    ...
    Socket.prototype.onack = function (packet) {
        ...
        if ('function' === typeof ack) {
            console.log('calling ack %s with %j', packet.id, packet.data);
        ...
        switch(data.type) {
            case 'ANSWER':
                sendData = {
                    ...
                    connectionType: data.payload.type
                }
                message = '42["SEND_ANSWER", ' + JSON.stringify(sendData) + ']';
            break;
            case 'CANDIDATE':
                sendData = {
                ...
                message = '42["SEND_CANDIDATE", ' + JSON.stringify(sendData) + ']';
            break;
            case 'OFFER':
                sendData = {
                    offer: data.payload.sdp,
                    dst: data.dst,
                    connectionId: data.payload.connectionId,
                    connectionType: data.payload.type,
                    dcInit: {},
                    ...
                    serialization: data.payload.serialization
                }
                message = '42["SEND_OFFER", ' + JSON.stringify(sendData) + ']';
            break;
            default:
                sendData = data.payload ? data.payload : null;
        }
    ...
    var util = {
        ...
        CLOUD_HOST: 'sign-toky-g-zd8f.webrtc.ecl.ntt.com',
        CLOUD_PORT: 443,
        ...
        supports: (function() {
            ...
            if (data) {
                ...
                // support reliable.
                var reliablePC = new window.RTCPeerConnection(defaultConfig, {});
                try {
            ...
            if (!onnegotiationneeded && data) {
                // sync default check.
                var negotiationPC = new window.RTCPeerConnection(defaultConfig, {optional: [{RtpDataChannels: true}]});
    ...
    function socketDataDecoder(obj) {
        var packet;
